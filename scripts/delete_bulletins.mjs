...
try {
  ...
  const collectionPath = "bulletins";
  const batchSize = 100;
  const collectionRef = db.collection(collectionPath);
  const query = collectionRef.orderBy("__name__").limit(batchSize);

  console.log(`🔥 Starting deletion of all documents in '${collectionPath}' collection...`);
  ...
  console.log(`✅ Successfully deleted all documents from '${collectionPath}' collection.`);
